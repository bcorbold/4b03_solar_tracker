...
void loop() {
  ...
  delay(1000);
  currentTime.dMinutes= currentTime.dMinutes + 10;
  ...
  if (currentTime.iDay >= 30){
    currentTime.iDay = 1;
    currentTime.iMonth++;
  }
  if (currentTime.iMonth >= 12){
    currentTime.iMonth = 0;
    currentTime.iYear++;
  }
}
...
void beginTracking() {
  ...
  if ((int)utcSunCoordinates.dZenithAngle > 00.1 && (int)utcSunCoordinates.dZenithAngle < 89.9) {
    ...
    Serial.print("Motor ZE= "); Serial.print((double)iStepsZE/(double)STEPSPERDEGZE);
    Serial.print(" Current iStepsZE= "); Serial.println(iStepsZE);
    Serial.print("utcSunCoordinates.dZenithAngle= ");
    Serial.println(utcSunCoordinates.dZenithAngle);  
    int deltaZenithSteps = (int)((90.0-utcSunCoordinates.dZenithAngle)*STEPSPERDEGZE) - iStepsZE; //store in getZenith result in variable
    MoveMotorZE(deltaZenithSteps);
    iStepsZE = (int)((90.0-utcSunCoordinates.dZenithAngle)*STEPSPERDEGZE);    
  } else {
